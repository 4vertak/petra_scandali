...
// смещение по осям x/y/z , где value_x/y/z - значения насколько должна быть
// смещена исходная точка
...
void rotation_y(data_t* data, double value) {
  ...
  for (size_t i = 0; i < 3 * num_vertices; i += 3) {
    ...
    double tmp_z = data->vertex[i + 2];
    data->vertex[i] =
        cos(value * (M_PI / 180)) * tmp_x - sin(value * (M_PI / 180)) * tmp_z;
    data->vertex[i + 2] =
        sin(value * (M_PI / 180)) * tmp_x - cos(value * (M_PI / 180)) * tmp_z;
  }
}
